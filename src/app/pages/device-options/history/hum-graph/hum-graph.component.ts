...
import { HumGraphData } from '@nd/core/models';

@Component({
    selector: 'nd-hum-graph',
    template: `
    <div [nbSpinner]="loading">
      <span *ngIf="!humData.length" class="hum--title-last">no logs</span>
      <div class="chart-container" #myChart></div>
    </div>
  `,
    styleUrls: ['./hum-graph.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush
})

export class HumGraphComponent implements AfterViewInit, OnDestroy {
    @ViewChild('myChart', { static: false })
    myChartRef: ElementRef;
    private alive = true;
    private humData$ = new ReplaySubject<HumGraphData[]>(1);
    @Input()
    loading: boolean;
    @Input()
    range: string;
    private _humData: HumGraphData[];
    @Input()
    set humData(value) {
        if (!!this.myChart) {
            this.myChart.clear();
        ...
        if (!!value && !!value.length) {
            this.humData$.next(value);
        }
        this._humData = value;
        console.log(value);
    }
    get humData() {
        return this._humData;
    }
    ...
    ngAfterViewInit() {
        ...
        this.humData$
            .pipe(withLatestFrom(this.theme.getJsTheme()), tap(([data, config]) => {
        ...
        if (this.myChart) {
            this.myChart.on('legendselectchanged', (event: any) => {
                const showRange = Object.keys(event.selected).every(key => event.selected[key]);
                this.option = {
                    ...this.option,
                    tooltip: {
                        ...this.option.tooltip,
                        formatter: `
                                    {c0} % </br>
                                    ${this.range !== 'day' && showRange ? 'Range: {c2} % - {c1} % </br>' : ''}  {b0}
                                   `
                    }
                };
                this.myChart.setOption(this.option);
    ...
    getHumiditySeries(humData: HumGraphData[], tTheme: any, range: string) {
        const series = {
            name: 'humidity',
            type: 'line',
            smooth: true,
            ...
            data: humData.map(i => i['hu'])
        };
        const { areaStyle, ...monthSeries } = series;
        return range === 'day' ? series : monthSeries;
    }
    getRangeHumSeries(data: HumGraphData[], tTheme: any, range: string) {
        return range === 'day'
            ? []
            : [];
    }
    getChartOption(humData: HumGraphData[], range: string, tTheme: any) {
        return {
            color: [
            ...
            tooltip: {
                ...
                formatter: `
                            {c0} % </br>
                            ${this.range !== 'day' ? 'Range: {c2} % - {c1} % </br>' : ''} {b0}
                           `,
            ...
            series: [
                this.getHumiditySeries(humData, tTheme, range),
                ...this.getRangeHumSeries(humData, tTheme, range)
            ],
            animationDelay: 100
        };
    }
